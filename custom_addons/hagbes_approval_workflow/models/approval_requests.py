...
class ApprovalRequest(models.Model):
    ...
    def _get_target_business_record(self):
        self.ensure_one()
        if not self.res_model or not self.res_id:
            return self.env['ir.model']

        model_exists = self.env['ir.model'].sudo().search([('model', '=', self.res_model)], limit=1)
        if not model_exists:
            _logger.warning("Approval request %s points to unknown model %s", self.id, self.res_model)
            return self.env['ir.model']
        ...
        try:
            ...
            _logger.exception(
                "Unable to load target record for approval request %s (%s, %s)",
                self.id,
                self.res_model,
                self.res_id,
            )
    ...
    def _run_target_callback(self, callback_name):
        self.ensure_one()
        record = self._get_target_business_record()
        if not record:
            _logger.warning(
                "Skipping approval callback %s for request %s because the target record no longer exists.",
                callback_name,
                self.id,
            )
            return

        callback = getattr(record, callback_name, None)
        if not callable(callback):
            _logger.debug(
                "Target record %s,%s has no callback %s",
                self.res_model,
                self.res_id,
                callback_name,
            )
        ...
        try:
            ...
            _logger.debug(
                "Target record %s,%s does not implement callback %s",
                self.res_model,
                self.res_id,
                callback_name,
            )

    def _sync_target_record(self):
        self.ensure_one()
        if self.status == 'approved':
            self._run_target_callback('_on_approval_approved')
        elif self.status == 'rejected':
            self._run_target_callback('_on_approval_rejected')

    def is_final_step(self):
        """Check if the current request is on its final step."""
        self.ensure_one()
        return self.current_step_id.is_final if self.current_step_id else False

    def get_next_step(self, action_type):
        """Helper to find what the next step would be for a given action."""
        self.ensure_one()
        if not self.current_step_id:
            return False
        step_action = self.current_step_id.action_ids.filtered(lambda a: a.action_id.code == action_type)[:1]
        return step_action.next_step_id if step_action else False

    def move_to_next_step(self, next_step):
        """Transition the request to a specific next step."""
        self.ensure_one()
        if not next_step:
            return False
        ...
        if next_step.is_final:
            ...
            if next_step.id not in self.completed_step_ids.ids:
                vals.update({'completed_step_ids': [(4, next_step.id)]})
        elif next_step.is_employee_step:
            if not self.requested_for_id:
                raise UserError(_("No 'Requested For' employee defined for this request."))
            vals.update({
                'status': 'pending',
        ...
        if next_step.is_final:
            self._sync_target_record()
        elif next_step.is_employee_step:
             self.activity_schedule(
                'mail.mail_activity_data_todo',
                user_id=self.requested_for_id.id,
                note=_("This request has been sent to you for review.")
            )
        elif next_step.is_condition:
    ...
    def process_action(self):
        ...
        # Check if current user is among them or has permission for this step
        if self.env.user not in approvers and not step.can_user_approve(self.env.user):
        ...
        if action_type == 'approve':
            if step.committee_approval:
                approved_count = self.env['approval.history'].search_count([
                    ('request_id', '=', self.id),
                    ('step_id', '=', step.id),
                    ('action_id', '=', action.id)
                ])
    ...
    def get_hierarchy_with_users_and_groups(self, job):
        ...
        while job:
            ...
            # Use getattr to safely access parent_id in case the extension is not loaded
            job = getattr(job, 'parent_id', self.env['hr.job'])
    ...
    def _check_org_chart(self, step):
        ...
        try:
            ...
            if not employee:
                _logger.warning("No employee record found for user %s", self.create_uid.id)

            hierarchy_info = self.get_hierarchy_with_users_and_groups(employee.job_id) if employee and employee.job_id else []
            ...
            while current_index < len(flow_steps):
                ...
                if current_step.is_organization:
                    if not employee or not employee.job_id:
                        raise UserError(
                            f"Cannot resolve organization-chart approvers for request {self.id} "
                            f"because the requester has no employee/job record."
                        )
                    ...
                    # Use getattr to safely access branch_id in case the extension is not loaded
                    elif not employee or not getattr(employee, 'branch_id', False):
                        matched_users = current_step.role_id.users
                    else:
                        matched_users = current_step.role_id.users.filtered(
    ...
    def _open_success_message_wizard(self, message):
        return {
            ...
            'view_mode': 'form',
            'view_id': self.env.ref('hagbes_approval_workflow.view_success_message_wizard').id,
            'target': 'new',
