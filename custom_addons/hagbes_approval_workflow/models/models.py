...
class ApprovalStep(models.Model):
    ...
    def can_user_approve(self, user):
        """Check if a specific user can approve this step."""
        self.ensure_one()
        ...
        # Security check: User must have the role assigned to this step
        if self.role_id:
            # Check if user has the group
            if user.has_group(self.role_id.get_external_id().get(self.role_id.id, '')):
                return True
            # Fallback for groups without XML ID in the current context
            if user in self.role_id.users:
                return True
            
        return False
        
    def can_user_reject(self, user):
        """Check if a specific user can reject this step."""
        return self.can_user_approve(user)

    def approve_step(self, request, user, comment=''):
        """Logic to approve this specific step for a given request."""
        self.ensure_one()
        if not self.can_user_approve(user):
            raise AccessError(_("You do not have permission to approve this step."))
        
        # Logic is mostly handled by request.process_action() in Odoo 18
        # but we provide this hook for custom step-level logic
        return True

    def reject_step(self, request, user, comment=''):
        """Logic to reject this specific step for a given request."""
        self.ensure_one()
        if not self.can_user_reject(user):
            raise AccessError(_("You do not have permission to reject this step."))
        return True
