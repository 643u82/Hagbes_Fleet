...
class FleetTrip(models.Model):
    """
    CONSOLIDATED Fleet Trip Model - Single Source of Truth for Execution Layer
    
    Combines functionality from:
    - fleet_allocation.py
    - fleet_trip_gps.py  
    - fleet_trip_log.py
    - fleet_vehicle_assign.py
    
    Responsibility: Vehicle assignment, execution tracking, GPS, logs
    """
    ...
    # ─── Assignment Details (consolidated from vehicle_assign) ─────────────
    ...
    # ─── Execution Tracking (consolidated from multiple models) ─────────────────
    ...
    # GPS Tracking (from fleet_trip_gps.py)
    ...
    # Trip Logs (from fleet_trip_log.py)
    ...
    def create(self, vals):
        ...
        # Auto-assign from requisition if provided
        if vals.get('requisition_id') and not vals.get('vehicle_id'):
            requisition = self.env['fleet.requisition'].browse(vals['requisition_id'])
            # You might want to implement vehicle selection logic here
            pass
    ...
    def action_start_trip(self):
        ...
        is_operator = self.env.user.has_group('hagbes_fleet.group_fleet_operator')
...
class FleetTripLog(models.Model):
    """
    CONSOLIDATED Trip Log Model - Single source for all trip logs
    
    Combines functionality from fleet_trip_log.py
    """
